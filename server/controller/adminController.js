...
export const adminSignup = async (req, res) => {
  ...
  //Check whether the admin with this email exist already
  let success = false;
  try {

    let admin = await Admin.findOne({ $or: [{ email: req.body.email }] });
    if (admin) {
      success = false;
      return res.status(400).json({ success, error: "Sorry a admin with this email or phone already exists" })
    }
...
export const updatedPassword = async (req, res) => {
  try {
    ...
    let hashedPassword;
    hashedPassword = await bcrypt.hash(newPassword, 9);
